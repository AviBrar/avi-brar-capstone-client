...
function SignIn() {
  ...
  return (
    <>
      <button className="chat__sign-in" onClick={signInWithGoogle}>Sign in with Google</button>
...
function SignOut() {
  return auth.currentUser && (
    <button className="chat__sign-out" onClick={() => auth.signOut()}>Sign Out</button>
  )
}
...
function ChatRoom() {
  ...
  return (<>
    ...
    <form onSubmit={sendMessage} className='chat__form'>
      ...
      <button type="submit" disabled={!formValue} className='chat__form-btn'>SEND</button>
...
function ChatMessage(props) {
  ...
  const messageClass = uid === auth.currentUser.uid ? 'chat__sent' : 'chat__received';
